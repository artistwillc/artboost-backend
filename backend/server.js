...
app.get("/privacy", (_req, res) => {
  return res.sendFile(`${process.cwd()}/website/privacy.html`);
});

app.get("/terms", (_req, res) => {
  return res.sendFile(`${process.cwd()}/website/terms.html`);
});
...
app.get("/favicon.png", (_req, res) => {
  return res.sendFile(`${process.cwd()}/website/favicon.png`);
});

app.get("/apple-touch-icon.png", (_req, res) => {
  return res.sendFile(`${process.cwd()}/website/apple-touch-icon.png`);
});
...
// ARTBOOST_SHOPIFY_AUTHORITATIVE_THUMBNAIL_REPAIR_V31644
const SHOPIFY_THUMBNAIL_REPAIR_VERSION =
  "V3.16.44-R1";
...
function getShopifyImageCandidates(node) {
  const featuredImageUrl =
    normalizeShopifyImageUrl(
      node?.featuredImage?.url
    );

  const productImagesImageUrl =
    normalizeShopifyImageUrl(
...
async function isReachableShopifyImage(
  imageUrl
) {
  const clean =
    normalizeShopifyImageUrl(imageUrl);
...
async function getShopifyPublicProductImage(
  shopDomain,
  handle
) {
  ...
  try {
    const response =
      await fetch(
        `https://${shopDomain}/products/${encodeURIComponent(
          handle
        )}.js`,
        {
          method: "GET",
...
// ARTBOOST_SHOPIFY_THUMBNAIL_PROXY_V31645
...
async function fetchShopifyImageBuffer(
  imageUrl
) {
  const clean =
    normalizeShopifyImageUrl(
...
async function loadLiveShopifyProductImage(
  connection,
  externalProductId
) {
  if (
  ...
  const query = `
    query ArtBoostShopifyProductImage(
      $id: ID!
    ) {
      product(id: $id) {
        id
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            success: false,
            error:
              "Missing userId or productId.",
            fixVersion:
              SHOPIFY_THUMBNAIL_PROXY_VERSION,
          });
      }
      ...
      const {
        ...
        await supabase
          .from("products")
          .select(
            "id,user_id,store_type,store_connection_id,external_product_id,image_url,product_url,metadata"
          )
          .eq(
            "id",
            String(productId)
      ...
      if (
        ...
        return res
          ...
          .json({
            success: false,
            error:
              "Shopify product image was not found.",
            fixVersion:
              SHOPIFY_THUMBNAIL_PROXY_VERSION,
          });
      }
      ...
      // If the stored CDN URL fails, resolve the product again from
      // Shopify Admin using the documented product(id:) query.
      for (
        const candidate of candidates
      ) {
        const image =
          await fetchShopifyImageBuffer(
            candidate
          );
        ...
        if (image) {
          ...
          res.setHeader(
            "X-ArtBoost-Shopify-Image-Source",
            "stored"
          );
          res.setHeader(
            "X-ArtBoost-Fix-Version",
            SHOPIFY_THUMBNAIL_PROXY_VERSION
          );
          return res
            .status(200)
      ...
      live =
        await loadLiveShopifyProductImage(
          connection,
          product.external_product_id
        );
      ...
      if (
        ...
        return res
          ...
          .json({
            success: false,
            error:
              "Shopify did not return a usable image for this product.",
            fixVersion:
              SHOPIFY_THUMBNAIL_PROXY_VERSION,
          });
      }
      ...
      console.error(
        "Shopify product image proxy failed:",
        error
      );
...
// ARTBOOST_SHOPIFY_DIRECT_IMAGE_FOUNDATION_V31660
function buildShopifyImageSourceCandidates(
  node
) {
...
function shopifyThumbnailPublicId(
  userId,
  externalProductId
) {
  const digest =
...
// ARTBOOST_SHOPIFY_DIRECT_IMAGE_SELECTION_V31660
function selectShopifyDirectImageV31660(
  node
) {
  const candidates =
...
// ARTBOOST_SHOPIFY_ORIGINAL_PRODUCTS_BACKEND_NORMALIZED_V31655
app.get("/shopify/products", async (req, res) => {
  try {
    ...
    const query = `
      query ArtBoostOriginalShopifyProductsV31660(
        $after: String
      ) {
        shop {
          currencyCode
    ...
    do {
      ...
      if (!shopifyResponse.ok) {
        console.error("Shopify products request failed:", shopifyResult);
    ...
    // V3.16.60 intentionally does not preserve prior thumbnail URLs.
    // Every Shopify Refresh overwrites image_url from Shopify directly.
    ...
    // ARTBOOST_SHOPIFY_ORIGINAL_DIRECT_PIPELINE_REBUILD_V31660
    const productsToSave =
      allProductEdges.map(
        ({ node }) => {
          ...
          const selectedImage =
            selectShopifyDirectImageV31660(
              node
            );
          ...
          return {
            ...
            metadata: {
              ...
              shopifyImageFoundation:
                "original-direct-shopify-v31660",
    ...
    for (
      ...
      const batch =
        productsToSave.slice(index,index + batchSize);
      ...
      if (upsertError) {
        console.error("Shopify product sync failed:", upsertError);
        return res.status(500).json({
          success: false,
          error:
    ...
    console.log(
      "ARTBOOST SHOPIFY DIRECT THUMBNAILS V31660",
      {
        store: connection.shop_domain,
        pages: pageCount,
        discovered: allProductEdges.length,
