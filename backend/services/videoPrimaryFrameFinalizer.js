// ARTBOOST_VIDEO_CONTRACT_V1_1
// Exact-duration normalization: short provider clips are safely extended to the
// requested motion window by holding the final generated frame, then trimmed.
...
const OPEN = 0.75, CLOSE = 0.75, TOTAL = 10, MOTION = TOTAL - OPEN - CLOSE;
...
function ffmpeg(args) {
  return new Promise((resolve, reject) => {
    ...
    p.on("error", reject);
    p.on("close", code => code === 0 ? resolve() : reject(new Error(`ffmpeg ${code}: ${err.slice(-3000)}`)));
  });
}
...
export async function finalizeVideoWithPrimaryImage({ job, primaryImageUrl, generatedVideoUrl, onProgress = async()=>{} }) {
  ...
  try {
    ...
    const filter = [
      ...
      // Runway/provider outputs can be shorter than requested. tpad holds the
      // generated clip's final frame long enough to fill the motion window;
      // trim then guarantees exactly MOTION seconds. No generated artwork is
      // regenerated, warped, or replaced.
    ...
    await ffmpeg([
      "-y", "-loop", "1", "-framerate", String(FPS), "-i", image, "-i", motion,
      "-filter_complex", filter,
      "-map", "[outv]", "-an", "-r", String(FPS), "-t", String(TOTAL),
    ...
    const uploaded = await cloudinary.uploader.upload(output, {
      ...
      overwrite: true,
      invalidate: true,
      tags: ["artboost", "video-studio", "exact-10-seconds", "primary-bookends", "duration-normalized"]
    });

    if (!uploaded?.secure_url) throw new Error("Cloudinary did not return a finalized video URL.");
    const d = Number(uploaded.duration);
    if (Number.isFinite(d) && Math.abs(d - TOTAL) > DURATION_TOLERANCE) {
      throw new Error(`Final video normalization failed: ${d}s instead of ${TOTAL}s.`);
    }
