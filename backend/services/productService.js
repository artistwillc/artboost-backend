...
// ARTBOOST_SHOPIFY_STOREFRONT_IMAGE_SELF_HEAL_V31650
const SHOPIFY_IMAGE_REPAIR_TIMEOUT_MS = 7000;
const SHOPIFY_IMAGE_REPAIR_CONCURRENCY = 6;
...
function getShopifyProductUrlParts(product) {
  if (
    String(
      product?.store_type || ""
    ).toLowerCase() !== "shopify"
...
async function fetchWithTimeout(
  ...
  const timeout =
    setTimeout(
      () =>
        controller.abort(),
      SHOPIFY_IMAGE_REPAIR_TIMEOUT_MS
    );
...
function extractImageFromProductJson(data) {
  const direct =
...
async function resolveShopifyStorefrontImage(
  product
) {
  const parts =
    getShopifyProductUrlParts(
      product
    );
      ...
      await fetchWithTimeout(
        `${origin}/products/${encodeURIComponent(handle)}.js`,
        {
          headers: {
            ...
            "User-Agent":
              "ArtBoostAI/1.0 Shopify Image Repair",
          },
        }
      );
  ...
  try {
    const response =
      await fetchWithTimeout(
        `${origin}/products/${encodeURIComponent(handle)}`,
        {
          headers: {
            Accept:
              "text/html,application/xhtml+xml",
            "User-Agent":
              "Mozilla/5.0 ArtBoostAI/1.0",
          },
        }
      );
...
async function repairMissingShopifyImage(
  product
) {
  if (
    !product ||
  ...
  const resolved =
    await resolveShopifyStorefrontImage(
      product
    );
  ...
  if (error) {
    console.warn(
      "Shopify storefront image self-heal save failed:",
      {
        productId:
          product.id,
        message:
  ...
  console.log(
    "ARTBOOST SHOPIFY IMAGE SELF-HEAL",
    {
      productId:
        product.id,
      title:
...
async function repairMissingShopifyImages(
  products
) {
  const rows =
  ...
  async function worker() {
    while (
      ...
      rows[current.index] =
        await repairMissingShopifyImage(
          current.product
        );
    }
  }
    ...
    Array.from(
      {
        length:
          Math.min(
            SHOPIFY_IMAGE_REPAIR_CONCURRENCY,
            indexes.length
          ),
      },
      () => worker()
...
export async function getProductById({
  ...
  return await repairMissingShopifyImage(
    product
  );
}
...
export async function getProducts({
  ...
  const repairedProducts =
    await repairMissingShopifyImages(
      products || []
    );
