...
export function isConsultantQuestionInScope({ question, conversation = [], hasImage = false } = {}) {
  ...
  // ARTBOOST_UNIVERSAL_ARTIST_SCOPE_V3_20260916
  const universalArtistDomain = /\b(?:tattoo|tattoos|tattooing|tattooist|tattooists|sculptor|sculptors|ceramic|ceramics|pottery|potter|woodwork|woodworking|woodcarving|metalwork|metalworking|blacksmith|blacksmithing|glasswork|glassworking|glassblower|glassblowing|fiber\s+art|textile|textiles|weaving|knitting|crochet|embroidery|quilting|leatherwork|leatherworking|jewelry|jewellery|jeweler|jeweller|silversmith|metalsmith|beading|resin\s+art|mosaic|printmaking|printmaker|screenprinting|screen\s+printing|linocut|woodcut|etching|engraving|calligraphy|calligrapher|lettering|mural|murals|muralist|graffiti|airbrush|airbrushing|craft|crafts|crafter|crafters|handmade|handcrafted|artisan|artisans|maker|makers|folk\s+art|installation\s+art|performance\s+art|conceptual\s+art|collage|assemblage|miniature|origami|bookbinding|floral\s+art|commission|commissions|booking|bookings|appointment|appointments|wholesale|consignment|art\s+fair|art\s+fairs|craft\s+fair|craft\s+fairs)\b/i;

  if (domain.test(q) || universalArtistDomain.test(q)) return true;
